...
def link_to_registry(model_name):
    ...
    with wandb.init(project="HRCSTagger", job_type="model_linking") as run:
        ...
        run.alert(f"Model {model_name} linked to registry at {MODEL_REGISTRY}.")
...
def stage_model(model_name):
    ...
    with wandb.init(project="grant_hrcs_tagger", job_type="staging") as run:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            proceed = input("Do you want to proceed and add the model to S3? (Y/N): ").strip().lower()
            ...
            if proceed == "y":
                ...
                artifact.save()
                run.alert(f"HRCSTagger model staged successfully at {s3_path}.")
...
def setup_parser():
    ...
    linking_parser = subparsers.add_parser("link", help="Link the model to the registry")
    linking_parser.add_argument("--model_name", type=str, help="Name of the model to link to the W&B registry.")

    staging_parser = subparsers.add_parser("stage", help="Stage the model for deployment")
    staging_parser.add_argument("--model_name", type=str, default=None, help="Name of the model to stage. If not provided, the latest model will be used.")
