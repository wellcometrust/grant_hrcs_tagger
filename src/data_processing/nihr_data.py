...
def read_nihr_dataset():
    ...
    clean_df = clean_df.loc[~clean_df["RA"].str.split().str[0].isin(["9.1", "9.2"])]
    ...
    clean_df["RA_top"] = clean_df["RA_top"].apply(
        lambda list: [x.strip()[:1] for x in list if pd.notnull(x) and x != "None"]
    )

    clean_df["RA"] = clean_df["RA"].str.split("/")

    clean_df["RA"] = clean_df["RA"].apply(lambda lst: [s.split()[0] for s in lst])
