...
def get_parent_categories(
    predictions, labels, detailed_label_names, parent_level="parent"
):
...
def log_metrics_to_wandb(
    prefix, label_names, indexed_len, metrics_dict,
    f1_per_class, precision, recall,
    label_count, prediction_count,
):
    """Log summary and per-class metric tables to W&B."""
    table = wandb.Table(
        columns=[
            "prefix",
            "precision_macro",
        ...
        data=[
            [
                prefix,
                metrics_dict[f"{prefix}precision_macro"],
                metrics_dict[f"{prefix}precision_micro"],
                metrics_dict[f"{prefix}recall_macro"],
                metrics_dict[f"{prefix}recall_micro"],
                metrics_dict[f"{prefix}f1_macro"],
                metrics_dict[f"{prefix}f1_micro"],
                label_count,
                prediction_count,
            ]
        ],
...
def calculate_metrics(labels, predictions, prefix=""):
    ...
    log_metrics_to_wandb(
        prefix, label_names, indexed_len, metrics_dict,
        f1_per_class, precision, recall,
        int(np.sum(labels[:, 0])), int(np.sum(predictions[:, 0])),
    )

    return metrics_dict


def _compute_parent_metrics(predictions, labels, detailed_metrics):
    """Compute parent and super-parent category metrics.

    Returns:
        tuple: (all_metrics dict, detailed_label_names dict)
    """
    ...
    return all_metrics, detailed_label_names


def _compute_wellcome_metrics(
    meta_path, labels, predictions, detailed_label_names, all_metrics
):
    """Compute Wellcome Trust filtered metrics and update all_metrics in place."""
    test_meta_path = os.path.join(meta_path, "test_meta.parquet")
    if not os.path.exists(test_meta_path):
        print(
            "Info: test_meta.parquet not found. Run train_test_split.py to generate metadata."
        )
    ...
    if "FundingOrganisation" not in meta_df.columns:
        print(
            "Info: FundingOrganisation column missing in meta; skipping Wellcome metrics."
        )
        wandb.log({"wellcome_metrics_status": "funding_org_missing"})
        return

    if len(meta_df) != labels.shape[0]:
        print(
            "Info: test_meta length does not match eval set; skipping Wellcome metrics."
        )
        wandb.log({"wellcome_metrics_status": "length_mismatch"})
        return

    wt_mask = (
        meta_df["FundingOrganisation"]
        .astype(str)
        .eq("Wellcome Trust")
        .to_numpy()
    )
    if not wt_mask.any():
        print(
            "Info: No rows for FundingOrganisation == 'Wellcome Trust' in eval set"
        )
    ...
    wt_detailed = calculate_metrics(
        labels_wt, predictions_wt, prefix="wellcome_"
    )
...
def _apply_predictions(logits, labels, config):
    """Convert logits to binary predictions using thresholding.

    Args:
        logits (np.array): Sigmoid-activated logits.
        labels (np.array): True labels.
    ...
    if config["training_settings"]["output_weighting"]:
        ...
        output_thresholds = config["training_settings"].get(
            "output_thresholds", [0.2, 0.5, 0.8, 0.95]
        )
    ...
    num_tags_predicted = np.sum(predictions, axis=1).tolist()

    if not config["training_settings"]["output_weighting"]:
        print(
            "num_tags_predicted: ",
...
def prepare_compute_metrics(config, meta_path: str | None = None):
    ...
    def compute_metrics(eval_pred):
        ...
        predictions = _apply_predictions(logits, labels, config)
        ...
        try:
            all_metrics, detailed_label_names = _compute_parent_metrics(
                predictions, labels, detailed_metrics
            )
        except (FileNotFoundError, ValueError, IndexError) as e:
            print(f"Warning: Could not calculate parent metrics: {e}")
            all_metrics = detailed_metrics
            detailed_label_names = None

        # Wellcome Trust filtered metrics
        if detailed_label_names is not None:
            try:
                _compute_wellcome_metrics(
                    meta_path, labels, predictions, detailed_label_names, all_metrics
                )
            except (FileNotFoundError, ValueError, IndexError) as e:
